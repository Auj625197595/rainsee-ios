...
public extension WKWebView {
  ...
  @MainActor func evaluateSafeJavaScriptThrowing(
    ...
    var sampledPageTopColor: UIColor? {
        ...
        // 通过 Selector 初始化一个选择器，用于调用私有方法 "_sampledPageTopColor"
        let selector = Selector("_sampledPageTopColor")
        
        // 检查当前对象是否能够响应指定的选择器
        if responds(to: selector),
            // 如果能够响应，则调用 perform 方法执行相应的选择器
            let result = perform(selector) {
                // 将执行结果进行非保留值的获取，并转换为 UIColor 类型
                return result.takeUnretainedValue() as? UIColor
        }
        
        // 如果无法响应选择器或者执行失败，则返回 nil
        return nil
    }

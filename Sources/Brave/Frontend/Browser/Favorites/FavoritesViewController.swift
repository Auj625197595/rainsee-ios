...
class FavoritesViewController: UIViewController {
  ...
  private func calculateAppropriateGrid() {
    ...
    if minWidth < 82 {
        itemSize = CGSize(width: floor(width / CGFloat(minimumNumberOfColumns)), height: FavoriteCell.height(forWidth: floor(width / CGFloat(minimumNumberOfColumns))))
...
extension FavoritesViewController: UICollectionViewDragDelegate, UICollectionViewDropDelegate {
  func collectionView(_ collectionView: UICollectionView, itemsForBeginning session: UIDragSession, at indexPath: IndexPath) -> [UIDragItem] {
    ...
    // 实现 UICollectionViewDropDelegate 协议的方法，处理拖放操作
    func collectionView(_ collectionView: UICollectionView, performDropWith coordinator: UICollectionViewDropCoordinator) {
        ...
        if let indexPath = coordinator.destinationIndexPath {
            ...
            // 如果目标位置为nil，说明拖放到了某个 section 的尾部
            // 获取 collectionView 最后一个 section 的索引
            let section = max(collectionView.numberOfSections - 1, 0)
            // 获取该 section 中的最后一个 item 的索引
            let row = collectionView.numberOfItems(inSection: section)
            // 设置目标位置为该 section 的最后一个 item 的下一个位置
            destinationIndexPath = IndexPath(row: max(row - 1, 0), section: section)
        }

        // 如果拖放源和目标位置的 section 不同，直接返回
        ...
        case .move:
            ...
            // 调用 Favorite 类的 reorder 方法，处理数据的重新排序
            Favorite.reorder(
...
extension FavoritesViewController {
  ...
  // 打开最近搜索的处理方法，接受一个RecentSearch对象作为参数
  func onOpenRecentSearch(_ recentSearch: RecentSearch) {
    // 调用recentSearchAction方法，传递recentSearch对象和false作为参数
  ...
  func onPasteboardAction() {
    // 调用recentSearchAction方法，传递nil和true作为参数
  ...
  func onRecentSearchHideOrClearPressed(_ button: UIButton) {
    ...
    if Preferences.Search.shouldShowRecentSearches.value {
      ...
      // 创建一个UIAlertController，用于提示用户是否确认清除最近搜索
      ...
      alert.addAction(
        UIAlertAction(
          ...
          style: .default,
          handler: { _ in
            // 确认清除最近搜索时，调用RecentSearch类的removeAll方法

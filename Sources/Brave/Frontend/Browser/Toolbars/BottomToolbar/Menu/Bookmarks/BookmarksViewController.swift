...
class BookmarksViewController: SiteTableViewController, ToolbarUrlActionsProtocol {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if Preferences.SyncRain.syncBook.value, timeStamp - Preferences.SyncRain.timeForBookMark.value > 100 {
            downloadBookmarks()
            Preferences.SyncRain.timeForBookMark.value = timeStamp
            print("执行downloadBookmarks")
    ...
    private func showEmptyPanelState() {
        if noSearchResultOverlayView.superview == nil {
            ...
            noSearchResultOverlayView.snp.makeConstraints { make in
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if !bookmark.isFolder {
            if tableView.isEditing {
                ...
                showEditBookmarkController(bookmark: bookmark)
            } else {
            ...
            if tableView.isEditing {
                ...
                updateLastVisitedFolder(bookmark)
    ...
    func tableView(_ tableView: UITableView, contextMenuConfigurationForRowAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        ...
        if bookmarkItem.isFolder {
            ...
            let urls: [URL] = getAllURLS(forFolder: bookmarkItem)
            ...
            let newPrivateTabAction = UIAction(
                ...
                handler: UIAction.deferredActionHandler { [unowned self] _ in
                    if !isPrivateBrowsing, Preferences.Privacy.privateBrowsingLock.value {
                        self.askForLocalAuthentication { [weak self] success, _ in
                            if success {
            ...
            let homeAction = UIAction(
                title: Strings.Home.sendToHomePageFavorites,
                image: UIImage(systemName: "heart.fill"),
                handler: UIAction.deferredActionHandler { [unowned self] _ in
                    Favorite.add(url: bookmarkItemURL, title: bookmarkItem.title!)
                })
            ...
            let linkMenu = UIMenu(title: "", options: .displayInline, children: [copyAction, shareAction, homeAction])
...
extension BookmarksViewController {
            ...
            if let children = item.children, !children.isEmpty {
                ...
                alert.addAction(
                    UIAlertAction(title: Strings.yesDeleteButtonTitle, style: .destructive) { _ in
                        ...
                        self.uploadLocalToNet(originMd5: "")
                        completion(true)
                    })
    ...
    private func showEditBookmarkController(bookmark: Bookmarkv2) {
        isEditingIndividualBookmark = true
        ...
        if let documentDirectory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first {
            ...
            if FileManager.default.fileExists(atPath: fileURL.path) {
                do {
                    ...
                    print("已存在的文件 \(fileName) 已被删除")
                } catch {
                    print("删除已存在的文件 \(fileName) 时发生错误：\(error)")
                }
            }
            ...
            do {
                ...
                print("文本成功保存到本地文件：\(fileName)")
            } catch {
                print("保存文本到本地文件时发生错误：\(error)")
            }
        }
    }


    func sendRequestWithLocalFile(json: String, md5: String) {
      
    }
    
    
    func uploadRequest(cookie: String, md5: String) {
       
    }

    fileprivate func downloadBookmark(_ jsonObjects: [[String: Any]]?, _ parentBook: Bookmarkv2, _ isRoot: Bool) {
        
    }

    fileprivate func uploadLocalToNet(originMd5: String) {
       
    }
    
    func decodeBase64AndParseJSON(base64EncodedString: String, originMd5: String) {
        ...
        do {
            ...
            downloadBookmark(jsonObjects, localMobileNode!, true)
...
//        }
    ...
    fileprivate func getBookmarkMd5(_ sorted: [Bookmarkv2]) -> String {
        var formatForMd5 = ""
        var lastUrl = ""
        for element in sorted {
            let url = element.url ?? ""
            if lastUrl != url {
                formatForMd5.append(url)
                formatForMd5.append(";")
                lastUrl = url
            }
        }
    ...
    func importBookmarks(from url: URL) {
        ...
        importExportUtility.importBookmarks(from: url) { [weak self] success in
            guard let self = self else { return }
    ...
    func exportBookmarks(to url: URL) {
        ...
        // 使用导入导出工具类导出书签到指定URL
        importExportUtility.exportBookmarks(to: url) { [weak self] success in
            // 在闭包中，确保使用弱引用避免循环引用
            guard let self = self else { return }
            ...
            if success {
                ...
                // 创建一个UIDocumentInteractionController以处理导出的文件
                // 控制器必须被保留，否则`AirDrop`和其他共享选项将失败！
                self.documentInteractionController = UIDocumentInteractionController(url: url)
                guard let vc = self.documentInteractionController else { return }

                // 设置UTI（Uniform Type Identifier）和文件名等属性
                vc.uti = UTType.html.identifier
                vc.name = "Bookmarks.html"
                vc.delegate = self

                // 在导出成功后，弹出共享选项菜单
                // 注意：importExportButton 必须保证有值，否则可能导致崩溃

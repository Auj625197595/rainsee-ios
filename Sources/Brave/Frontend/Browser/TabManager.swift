...
//        // 在函数返回之前执行心跳检测和添加到 RunLoop 的操作
//        defer {
//            metricsHeartbeat?.fire() // 触发心跳检测
//            RunLoop.current.add(metricsHeartbeat!, forMode: .default) // 将心跳检测添加到RunLoop
//        }
        
        // 标记正在进行还原操作
        isRestoring = true
        
        // 调用内部方法进行标签还原，返回还原的标签
        ...
        return tabToSelect ?? self.addTab(isPrivate: isPrivate) // 返回选择的标签，如果没有则添加新标签并返回
    }
...
extension TabManager: NSFetchedResultsControllerDelegate {
  ...
  // NSFetchedResultsControllerDelegate协议方法，处理Core Data数据变更
  func controller(_ controller: NSFetchedResultsController<NSFetchRequestResult>, didChange anObject: Any, at indexPath: IndexPath?, for type: NSFetchedResultsChangeType, newIndexPath: IndexPath?) {
    
    // 检查被修改的对象是否是Domain类型，并获取其URL
    if let domain = anObject as? Domain, let domainURL = domain.url {
      
      // 如果`wallet_permittedAccounts`在`Domain`上发生变化
      // 从钱包设置/管理web3站点连接，我们需要在打开的标签上触发`accountsChanged`事件
      ...
      tabsForDomain.forEach { tab in
        Task { @MainActor in
          ...
          // iOS没有`HostContentSettingsMap`，所以我们必须实现`SolanaProviderImpl::OnContentSettingChanged`
//          if let selectedSolAccount = allAccounts.solDappSelectedAccount,
//             tab.isSolanaAccountConnected(selectedSolAccount.address), // 当前已连接
//             !tab.isAccountAllowed(.sol, account: selectedSolAccount.address) { // 用户撤销了访问权限
//            tab.walletSolProvider?.disconnect()
//          }
          
//          // 获取以太坊账户地址列表
//          let ethAccountAddresses = allAccounts.accounts.filter { $0.coin == .eth }.map(\.address)
//          
//          // 获取标签允许的以太坊账户地址列表
//          let allowedEthAccountAddresses = tab.getAllowedAccounts(.eth, accounts: ethAccountAddresses) ?? []
          
          // 触发标签的`accountsChanged`事件

...
//    return Static.Section(header: .view(header))
    ...
    private func makeFeaturesSection() -> Static.Section {
        ...
        var section = Static.Section(
            ...
            rows: [
                Row(
                    text: Strings.Other.adBlockandPrivacyProtection,
                    selection: { [unowned self] in
...
//                            }
//
//                            // 创建 SyncSettingsTableViewController 实例
...
//                            )
//
//                            // 推送 SyncSettingsTableViewController 到导航控制器堆栈
//                            self.navigationController?.pushViewController(syncSettingsViewController, animated: true)
//                        } else {
//                            // 创建 SyncWelcomeViewController 实例
...
//                            )
//
//                            // 推送 SyncWelcomeViewController 到导航控制器堆栈
...
//      cellClass: ColoredDetailCell.self, context: [ColoredDetailCell.colorKey: color], uuid: "vpnrow")
    ...
    private lazy var securitySection: Static.Section = Section(
    ...
    private lazy var supportSection: Static.Section = .init(
...
//                        }
                ...
                Row(
                    ...
                    selection: { [unowned self] in
                        // 显示隐私政策
                        let privacy = SettingsContentViewController().then { $0.url = .brave.privacy_h5 }
                ...
                // 联系我们
                Row(
                    text: Strings.Other.contactUs,
                    selection: { [unowned self] in
                        // 显示许可证信息
                        let licenses = SettingsContentViewController().then {
                            $0.url = URL(string: URL.brave.contact_us)
                        }
                        self.navigationController?.pushViewController(licenses, animated: true)
                    }, accessory: .disclosureIndicator),
                // 联系我们
                Row(
                    text: Strings.Other.updateLogandFeedback,
                    selection: { [unowned self] in
                        // 显示许可证信息
                        let licenses = SettingsContentViewController().then {
                            $0.url = URL(string: URL.brave.update_log)
                        }
                        self.navigationController?.pushViewController(licenses, animated: true)
                    }, accessory: .disclosureIndicator),
                // 备案号
                Row(
                    text: "备案号(浙ICP备2022027117号-4A)",
                    selection: { [unowned self] in
                        // 显示许可证信息
                        UIPasteboard.general.strings = ["浙ICP备2022027117号-4A"]
                        let alertController = UIAlertController(title: Strings.Other.alertTitle, message: Strings.Other.copySuccessMessage, preferredStyle: .alert)
    ...
    private func setUpSections() {
        var copyOfSections = makeSections()
        ...
        dataSource.sections = copyOfSections
    }
